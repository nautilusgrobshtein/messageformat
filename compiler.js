...
Compiler.prototype.compile = function(src, lc, plurals) {
	if (typeof src != 'object') {
		...
		var parsed = parse(src, pc);
		var hasReact = hasReactToken(parsed);

		var r = parsed.map(function(token) {
			return this.token(token);
		}, this);

		var output;
		if (hasReact) {
			var reactParsed = r.map(function(token) {
				if (token.type === "react") {
					return 'd.' + token.arg;
				} else if (token.type === "reactWrap") {
					return 'React.cloneElement(d.' + token.arg + ', null, ' + JSON.stringify(token.text) + ')';
				}

				return token;
			}, this);

			output = 'React.createElement(React.Fragment, null, ' + (reactParsed.join(', ') || 'null') + ')';
